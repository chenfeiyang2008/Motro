...
- The learner chrome is the only regular Liquid Glass layer: a bottom-safe-area-aware capsule Dock on compact layouts, a left-edge-attached side rail on desktop, and a compact header when the Dock is present. It uses an opaque high-contrast fallback for reduced transparency, increased contrast and unsupported backdrop effects.
